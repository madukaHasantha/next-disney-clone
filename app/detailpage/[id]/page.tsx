...
async function getData(id: string) {
  try {
    const res = await fetch(
      `https://disney-7a649-default-rtdb.asia-southeast1.firebasedatabase.app/movie/${
        +id - 1
      }.json`,
      { cache: "no-store" }
    );
...
async function detailpage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="detailpageContainer">
      ...
      <div className="detailpageSubTitle">{movies.subTitle}</div>

      <div className="detailpageDescription">{movies.description}</div>
    </div>
  );
}
